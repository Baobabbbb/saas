...
class RunwayGen4ProductionService:
    ...
    async def _generate_image_from_text(self, prompt: str) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:                payload = {
                    "model": "gen4_image",  # Modèle correct pour la génération d'image
                    "promptText": prompt,
                    "ratio": "1280:720",
                    "seed": None
                }
    ...
    async def _generate_video_from_image(self, image_url: str, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                payload = {
                    "model": "gen3a_turbo",  # Modèle Gen-4 Turbo
                    "prompt": prompt,
                    "image": image_url,
                    "duration": 10,
                    "ratio": "16:9",
    ...
    async def _poll_video_status(self, task_id: str, max_attempts: int = 60) -> Optional[Dict[str, Any]]:
        """Poll le statut de la génération vidéo"""
        
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                for attempt in range(max_attempts):
                    print(f"🔄 Vérification statut vidéo ({attempt + 1}/{max_attempts})...")
                    ...
                    if response.status_code == 200:
                        ...
                        if status == "SUCCEEDED":
                            ...
                            print(f"❌ Génération vidéo échouée: {result.get('failure_reason', 'Raison inconnue')}")
                            return None
                        
                        elif status in ["PENDING", "RUNNING"]:
                            print(f"⏳ Génération en cours... ({status})")
                ...
                print("⏰ Timeout: Génération vidéo trop longue")
                return None
                
        except Exception as e:
            print(f"❌ Exception polling vidéo: {str(e)}")
            return None
    ...
    async def generate_animation(self, animation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            video_result = await self._poll_video_status(video_task["task_id"])

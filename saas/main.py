...
# Endpoint pour réinitialiser le mot de passe (via Resend direct)
@app.post("/api/reset-password")
async def reset_password_endpoint(reset_data: PasswordResetRequest):
    """Réinitialise le mot de passe via Resend directement pour meilleure délivrabilité"""
    try:
        resend_api_key = os.getenv("RESEND_API_KEY")
        ...
        reset_url = f"{BASE_URL}/reset-password?token={reset_token}&email={reset_data.email}"

        # Template HTML professionnel pour éviter les spams
        html_content = f"""
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Réinitialisation de votre mot de passe Herbbie</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <div class="header">
            <div class="logo">HERBBIE</div>
            <h1>Réinitialisation de votre mot de passe</h1>
        </div>

        <p>Bonjour,</p>

        <p>Vous avez demandé la réinitialisation de votre mot de passe pour votre compte Herbbie.</p>
        ...
        <div class="warning">
            <strong>Attention :</strong> Ce lien est valable pendant 1 heure. Si vous n'avez pas demandé cette réinitialisation, ignorez cet email.
        </div>

        <p>Si le bouton ne fonctionne pas, copiez et collez cette URL dans votre navigateur :</p>
        <p style="word-break: break-all; background: #f8f9fa; padding: 10px; border-radius: 5px; font-family: monospace; font-size: 12px;">{reset_url}</p>

        <p>Cordialement,<br>L'équipe Herbbie</p>

        <div class="footer">
            <p>Cet email a été envoyé automatiquement. Ne pas répondre à cet email.</p>
            <p>© 2025 Herbbie - Tous droits réservés</p>
        </div>
    </div>
</body>
</html>
"""
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                print(f"✅ Email de reset envoyé avec succès à {reset_data.email}")
                return {"message": "Email de réinitialisation envoyé avec succès"}
            else:
                error_msg = response.text
                print(f"❌ Erreur Resend reset password ({response.status_code}): {error_msg}")
                raise HTTPException(status_code=500, detail=f"Erreur lors de l'envoi: {error_msg}")
